...
<head>
    ...
    <script type="text/javascript">
            ...
            $submit.click(function() {
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                       ...
                       /* 用于在向服务器发送请求前添加一些处理函数。这是一个ajax事件，
                        在ajax请求开始之前就被触发，通常允许用户修改XMLHttpRequest对象（比如说设置附加的头部信息）*/
                    },
        ...
        $("#sendVerifyCodeBtn").click(function(){
             ...
             //发送成功，
             //disabled不使能，如果为true，就是禁止，false，就是允许
             $("#sendVerifyCodeBtn").prop('disabled',true);
             $.get("${ctx}/sms/send",{'p，hone':phone},function(data){
                    if(data.resultCode==0){
                        alert(data.resultMessage);
                        $("#sendVerifyCodeBtn").prop('disabled',false);//发送失败时，允许继续发送
                    }
                 downtime();//进入倒计时
                });
    ...
    function downtime(){
     ...
     /*   function countdownInterval() {
            ...
            var interval = setInterval(function() {
                if (countTime == 0 && interval != null) {
                    clearInterval(interval);
                    countTime = 60;
                    $("#sendVerifyCodeBtn").val("重新发送");
                    $("#sendVerifyCodeBtn").attr('disabled', false);
                } else {
                    $("#sendVerifyCodeBtn").val(countTime + "s");
                    countTime--;
                }
            }, 1000);
        }*/
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#captchaImage").click(function() {
            $(this).hide().attr('src',
                    "${ctx}/kaptcha/getKaptchaImage?" + new Date().getTime()).fadeIn(); // 加载时间戳去浏览器缓存
        });
    });
</script>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="fieldSet">
                                     ...
                                     <img id="captchaImage" style="width:80px;height:30px" class="captchaImage"
                                     src="${ctx}/kaptcha/getKaptchaImage" title="点击更换验证码">
                                    </span>
                                </td>
                            </tr>
                            <tr>
